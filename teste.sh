...
patch_version_mk() {
    ...
    sed -i '/^LINEAGE_VERSION_SUFFIX := .*/a \ \ # Add MICROG to suffix if WITH_GMS is true\ ifeq ($(WITH_GMS),true)\ LINEAGE_VERSION_SUFFIX := $(LINEAGE_VERSION_SUFFIX)-MicroG\ endif\ \ # Add custom build tag/feature to suffix if BUILD_TAG is defined\ ifneq ($(BUILD_TAG),)\ LINEAGE_VERSION_SUFFIX := $(LINEAGE_VERSION_SUFFIX)-$(BUILD_TAG)\ endif' "$version_mk"
    if grep -q "MICROG" "$version_mk"; then
...
cat > /tmp/remove_dup_protobuf.py << 'EOF'
import re

path = "hardware/lineage/compat/Android.bp"
names = [
...
for name in names:
    ...
    if count == 0:
        print(f"[AVISO] bloco '{name}' nao encontrado, nada removido")
    else:
        print(f"[OK] bloco '{name}' removido ({count}x)")
    content = new_content

with open(path, "w") as f:
    f.write(content)
EOF
python3 /tmp/remove_dup_protobuf.py
...
if [ -n "$ROM_NAME" ]; then
    ...
    cat > ~/LineageOS-MicroG/gofile << 'GEOFILESCRIPT'
#!/bin/bash
...
GEOFILESCRIPT
    ...
    chmod +x ~/LineageOS-MicroG/gofile
    
    ROM_OUTPUT=$(~/LineageOS-MicroG/gofile "$ROM_PATH" "LineageOS 23.2 MicroG - sapphire")
    if [ $? -eq 0 ]; then
